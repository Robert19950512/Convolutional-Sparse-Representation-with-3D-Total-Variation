...
def GradientFilters(ndim, axes, axshp, dtype=None):
    ...
    G4 = np.eye(axshp[0],axshp[1])-np.insert(np.delete(-np.eye(axshp[0],axshp[1]),axshp[0]-1,1),0,np.zeros(axshp[0]),1)
    G4f = rfftn(G4,axshp,axes=axes)
    dim = Gf.shape
    Gf = np.append(Gf,G4f).reshape(dim[0],dim[1],dim[2],dim[3],dim[4],dim[5]+1)
